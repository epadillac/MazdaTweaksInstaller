...
function List2Ctrl(uiaId, parentDiv, controlId, properties)
{
    ...
    this.items              = new Array(); // {array} holds currently displayed list items. Usually these extend the visible range

    // letter index
    this.letterIndexData    = new Array(); // {array} holds letter index data
    ...
    this._letterIndexDataSorted = new Array(); // {array} holds sorted letter index data
...
List2Ctrl.prototype.init = function()
{
    ...
    var loadingImage1 = document.createElement('span');
...
{
    this.pool = {
        empty : new Array(),        // 'empty' is internal style
        draggable : new Array(),    // 'draggable' is internal style
        ghost : new Array(),        // 'ghost' is internal style
        ...
        style25 : new Array(),
        styleOnOff : new Array(),   // not official name
    ...
    {
        for (var j=0; j<this.properties.poolsize; j++)
        {
...
List2Ctrl.prototype._prepareItems = function(firstItem, lastItem)
{
    ...
    for (var i=firstItem, l=lastItem; i<=l; i++)
    {
    ...
    for (var i=firstItem, l=lastItem; i<=l; i++)
    {
        switch (this.dataList.items[i].itemStyle)
        {
            ...
            case 'draggable' :
                ...
                {
                    var button1 = this._getLocalizedString(this.dataList.items[i].button1Id, this.dataList.items[i].button1SubMap);
            ...
            case 'style05' :
                ...
                {
                    var text2 = this._getLocalizedString(this.dataList.items[i].text2Id, this.dataList.items[i].text2SubMap);
            ...
            case 'style10' :
                ...
                {
                    var button2 = this._getLocalizedString(this.dataList.items[i].button2Id, this.dataList.items[i].button2SubMap);
            ...
            case 'style11' :
                ...
                {
                    var button3 = this._getLocalizedString(this.dataList.items[i].button3Id, this.dataList.items[i].button3SubMap);
...
List2Ctrl.prototype._setSlider = function(li, className, sliderProperties, itemIndex)
{
    ...
    var sliderCont = li.querySelector(className);
    if (sliderProperties && sliderCont)
    {
        var hashKey = 'slider_'+itemIndex+'_'+poolId;
...
List2Ctrl.prototype._setToggle = function(li, className, value)
{
    ...
    var buttons = li.querySelectorAll(className);
...
List2Ctrl.prototype._getListItem = function(listItem, dataListIndex)
{
    ...
    var li = this.pool[listItem.itemStyle].shift();
...
{
    // get the style
    var itemStyle = li.getAttribute('data-itemStyle');
    // reset it
    li.style.top = '0px';
    // remove it
...
List2Ctrl.prototype._updateRange = function()
{
    ...
    var itemsBefore = this.properties.itemsBefore;
    var itemsAfter = this.properties.itemsAfter;
    var topItem = this._topItem;
    var numOfScrolledElements = topItem - this._prevTopItem;
    var itemsOnScreen = itemsBefore + itemsAfter + 1;
    ...
    if (this.m.abs(numOfScrolledElements) > itemsOnScreen)
    {
            ...
            topItem > Math.round((itemsOnScreen / 2)) )
        {
            ...
            for (var i=0; i < itemsLength; i++)
            {
                ...
                // get it from pool
                var li = this._getListItem(this.dataList.items[dataListIndex], dataListIndex);
    ...
    else
    {
        ...
        if (numOfScrolledElements > 0)
        {
            ...
            // return to pool
            var firstItemRef = this.items[0].ref;
            var bottomDifference = topItem - firstItemRef;
            var extraEls = bottomDifference - itemsBefore;
            ...
            // get from pool
            var topDifference = this.items[this.items.length-1].ref - ( topItem - 1 );
            var newEls = ( itemsAfter + 1 ) - topDifference;
            ...
            if (newEls > 0)
            {
                ...
                for (var i=0; i<newEls; i++)
                {
                    // index in the dataList
                    var dataListIndex = this.items[this.items.length-1].ref + 1;
        ...
        else if (numOfScrolledElements < 0)
        {
            ...
            // return to pool
            var topDifference = this.items[this.items.length-1].ref - topItem + 1;
            var extraEls = topDifference - ( itemsAfter + 1 );
            ...
            // get from pool
            var bottomDifference = topItem - this.items[0].ref;
            var newEls = itemsBefore - bottomDifference;
            ...
            if (newEls > 0)
            {
                ...
                {
                    // index in the dataList
                    var dataListIndex = this.items[0].ref - 1;
...
List2Ctrl.prototype._updateDisplay = function(firstItem, lastItem)
{
    ...
    var firstItemRef = this.items[0].ref;
    var lastItemRef = this.items[this.items.length-1].ref;
        ...
        (firstItem <= firstItemRef && lastItem >= firstItemRef) )
    {
        ...
        var firstToUpdate = this.m.max(firstItem, firstItemRef);
        var lastToUpdate = this.m.min(lastItem, lastItemRef);
        var firstToUpdateIndex = firstToUpdate - firstItemRef;
        var lastToUpdateIndex = (lastToUpdate - firstToUpdate) + firstToUpdateIndex;

        for (var i=firstToUpdateIndex; i<=lastToUpdateIndex; i++ )
        {

            var returnItem = this.items.splice(i,1);
            var dataListIndex = returnItem[0].ref;
...
List2Ctrl.prototype._fill = function(firstItem, lastItem)
{
    ...
    {
        // get it from the pool
        var li = this._getListItem(this.dataList.items[i], i);
    ...
    // update _isScrollable flag
    var scrollable = this._checkScrollable();
...
List2Ctrl.prototype._setTopListItem = function(pos)
{
    ...
    if (this.properties.enableItemRequestOnScroll)
    {
        ...
        var emptyDOMItem = this._getEmptyDOMElement();
...
List2Ctrl.prototype._dragUpdateScrollIndicator = function()
{
    ...
    var indicatorPos = Math.round(this._indicatorMax * (this.scroller.offsetTop / this._maxScrollY));
...
List2Ctrl.prototype._start = function(e)
{
    ...
    var relativeX = e.pageX - this._maskPositionX;
    ...
    if (relativeY >= 0)
    {
        ...
        // steal focus
        var canGainFocus = this._canGainFocus(e);
    ...
    {
        var temp = this._currentSecondaryMulticontrollerItem;
...
List2Ctrl.prototype._move = function(e)
{
    ...
    var deltaX = relativeX - this._startX;
    ...
    if (null == this._inHorizontalDrag) {
        ...
        var alpha = Math.atan2(this.m.abs(deltaX), this.m.abs(deltaY));
    ...
    else if (false == this._inHorizontalDrag && this._isScrollable)
    {
        ...
        var newPos = this.m.max(this._maxScrollY, this.m.min(this._y + deltaY, this._minScrollY));
    ...
    {
        var targetTop = this._startDOMItem.offsetTop;
...
List2Ctrl.prototype._end = function(e)
{
    ...
    // detect swipe motion
    var endTime = e.timeStamp || new Date().getTime();
    ...
    if (this._focusStolen && !this._stopSelect)
    {
        ...
        var focussedIndex = this._getFocussedIndex();
        var allowOffScreen = (focussedIndex > this._topItem && focussedIndex < this._topItem + this.properties.visibleItems);
...
List2Ctrl.prototype._moveIndex = function(e)
{
    ...
    // calculate travelled distance
    var deltaY = relativeY - this._startIndexY;
    var deltaX = relativeX - this._startIndexX;

    // calculate the letter index's new position and constrain it into bounds
    var newPos = this.m.max(this._maxScrollYIndex, this.m.min(this._yIndex + deltaY, this._minScrollYIndex));
...
List2Ctrl.prototype._endIndex = function(e)
{
    ...
    if (!this._stopSelect)
    {
        ...
        // select letter index
        var letterIndex = this._getTargetLetterIndex(e);
    ...
    else
    {
        // detect swipe motion
        var endTime = e.timeStamp || new Date().getTime();
        var velocity = endTime - this._startTimeIndex;
...
List2Ctrl.prototype._touchSelectItem = function(e)
{
    ...
    if (this._isToggle(itemIndex))
    {
        ...
        var toggleSelected = this._buttonSelect(e);
    ...
    if (this._isStep(itemIndex) && this._hasSecondaryMulticontroller(itemIndex) && this._inSecondaryMulticontroller)
    {
        ...
        var stepResult = this._stepAdjust(e);
    ...
    if (this._isLock(itemIndex) && this._hasSecondaryMulticontroller(itemIndex))
    {
        ...
        var lockAction = this._lockSelect(e);
        if (null == lockAction)
        {
            fireSelect = false;
        }
...
{
    var hitItems = this.scroller.querySelectorAll('.hit');

    if (hitItems.length)
    {
        for (var i=0, l=hitItems.length; i<l; i++)
        {
...
List2Ctrl.prototype._itemMakeHit = function(e)
{
    ...
    var target = this._getDOMItem(itemIndex);
...
{
    var longpressItems = this.scroller.querySelectorAll('.longpress');

    if (longpressItems.length)
    {
        for (var i=0, l=longpressItems.length; i<l; i++)
        {
...
List2Ctrl.prototype._simpleSelect = function(itemIndex)
{
    ...
    switch (this.dataList.items[itemIndex].itemStyle)
    {
        case 'styleOnOff' :
            ...
            var currentValue = this.dataList.items[itemIndex].value;
            var newValue = (1 === currentValue) ? 2 : 1;
            this.dataList.items[itemIndex].value = newValue;

            // get and update DOM item
            var domItem = this._getDOMItem(itemIndex);
            if (domItem)
            {
                if (1 === newValue)
                {
        ...
        case 'style03' :
        case 'style03a' :
            var currentValue = this.dataList.items[itemIndex].checked;
...
List2Ctrl.prototype._itemSelect = function(itemIndex, paramsModifier, additionalModifier)
{
    ...
    // get paramsModifier
    var paramsModifier = paramsModifier || {};

    // get additionalModifier
    var additionalModifier = additionalModifier || null;
    ...
    if (this._isSimpleSelectItem(itemIndex))
    {
        ...
        // apply event filter
        var filterType = (this._isToggle(itemIndex) || this._isOnOff(itemIndex)) ? 'toggle' : (this._isCheckBox(itemIndex)) ? 'check' : null;
...
List2Ctrl.prototype._itemLongPress = function(e)
{
    ...
    {
        // determine target item
        itemIndex = this._getTargetItem(e);var itemIndex = this._getTargetItem(e);
        eventCause = 'Touch';
    }
    // the parameter is an index
    else if (!isNaN(e))
...
List2Ctrl.prototype._settleItem = function(itemIndex)
{
    ...
    var item = this.dataList.items[itemIndex];
    ...
    var settleValue = item._data.settleValue;
    ...
    if (!this._inSecondaryMulticontroller)
    {
        ...
        switch (eventID)
        {
            case "acceptFocusInit":
                ...
                {
                    this._hasFocus = true;
                    var itemToGainFocus = this._canGainFocus('controllerActive');
            ...
            case "acceptFocusFromLeft":
                ...
                var itemToGainFocus = this._canGainFocus();
                if (-1 !== itemToGainFocus)
                {
                    this._hasFocus = true;
                    this._showFocus(itemToGainFocus);
            ...
            case "down" :
                ...
                if ('downStart' === this._lastControllerEvent)
                {
                    ...
                    if (this._inLetterIndexMulticontroller)
                    {
                        var currentFocussedLetterIndex = this._getFocussedLetterIndex();
...
List2Ctrl.prototype._handleMCCW = function()
{
    ...
    if (this._inLetterIndexMulticontroller)
    {
        ...
        var rfi = this._getRelativeFocussedLetterIndex();

        // define threshold that will serve as a scroll trigger
        var bottomFocusThreshold = this.properties.visibleLetterIndexItems - 2;
    ...
    else
    {
        ...
        else
        {
            ...
            if (this._getFocussedIndex() == currentFocussedIndex) //didn't move
            {
                if (this.dataList && this.dataList.items)
                {
                    var index = this.dataList.items.length - 1;
...
List2Ctrl.prototype._setSecondaryMulticontroller = function(state, focussedIndex)
{
    ...
    if (state)
    {
        ...
        var domItem = this._getDOMItem(focussedIndex);
        if (domItem)
        {
...
List2Ctrl.prototype._showFocus = function(item, allowOffscreen, simulationMode, abortMode)
{
    ...
    var simulationMode = (true === simulationMode);

    // get the last focussed index (real and relative)
    var lastFocussedIndex = this._getFocussedIndex();
    var lastRelativeFocussedIndex = this._getRelativeFocussedIndex();
    ...
    var nextFocussedIndex = -1;
    var useTransition = true;
    var useRelativeIndeces = true;
    ...
    switch (item)
    {
        case 'down' :
            ...
            if (!simulationMode)
            {
                var nextRealFocussedIndex = this.m.min(lastFocussedIndex+1, this.dataList.itemCount-1);
        ...
        case 'up' :
            ...
            if (!simulationMode)
            {
                var nextRealFocussedIndex = this.m.max(lastFocussedIndex-1, 0);
                while(this.dataList.items[nextRealFocussedIndex].disabled)
                {
    ...
    var focussedElement = this._getDOMItem(nextFocussedIndex);
...
List2Ctrl.prototype._triggerFocus = function(itemIndex)
{
    ...
    // get the index
    var focussedIndex = (!isNaN(itemIndex))? itemIndex : this._getFocussedIndex();
    ...
    if (focussedIndex != null)
    {
        ...
        if (this._isLock(focussedIndex))
        {
            var focussedButton = this._lockGetFocus(focussedIndex);
            var actionResult = this._lockActivate(focussedIndex, focussedButton);
            this._lockShowFocus(focussedIndex, 'clear');
...
List2Ctrl.prototype._canGainFocus = function(e)
{
    ...
    var itemToGainFocus = -1;

    // check if we are touching the element
    if (typeof e === 'object')
    {
        var targetItem = this._getTargetItem(e);
        ...
        {
            var nearestItem = this._getNearestEnabledItem(targetItem);
    ...
    else if (typeof e === 'number')
    {
        ...
        {
            var nearestItem = this._getNearestEnabledItem(e);
            itemToGainFocus = (null != nearestItem) ? nearestItem : -1;
        }
    }
    ...
    else
    {
        ...
        var nearestItem = this._getNearestEnabledItem(this._lastItemWithFocus);
        itemToGainFocus = (null != nearestItem) ? nearestItem : -1;
...
{
    var focussedIndex = this._lastItemWithFocus;
...
{
    var direction = (direction != 'up' && direction != 'down') ? 'down' : direction;
    var currentItem = ('down' === direction) ? fromItem+1 : fromItem-1;
...
{
    var nearestEnabledItem = null;

    var nearestDown = this._getNearestEnabledItemByDirection(fromItem, 'down');
    var nearestUp = this._getNearestEnabledItemByDirection(fromItem, 'up');
    ...
    {
        var differenceDown = this.m.abs(fromItem - nearestDown);
        var differenceUp = this.m.abs(fromItem - nearestUp);
...
List2Ctrl.prototype._hasSecondaryMulticontroller = function(itemIndex)
{
    ...
    var hasSecondaryMulticontroller = false;

    var type = this.dataList.items[itemIndex].itemStyle;
    for (var i=0; i<this._secondaryMulticontrollerTypes.length; i++)
    {
...
{
    var newPos = -(index) * this.properties.itemHeight;                 // calculate new position based on the item index
...
List2Ctrl.prototype._scrollDownOne = function()
{
    ...
    {
        var bi = this._getNearestEnabledItemByDirection(this._topItem+this.properties.visibleItems-2, 'down');
        // do not scroll if no enabled items are found
        if (null != bi)
        {
            var newTopItem = bi + 2 - this.properties.visibleItems;
...
List2Ctrl.prototype._scrollUpOne = function()
{
    ...
    {
        var bi = this._getNearestEnabledItemByDirection(this._topItem+1, 'up');
        // do not scroll if no enabled items are found
        if (null != bi)
        {
            var newTopItem = bi - 1;
...
{
    // get list position
    var listPosition = this._getListPosition();

    // set return status
    var returnStatus = 'onepage';
    ...
    switch (listPosition)
    {
        ...
        case 'bottom' :
            ...
            var nei = this._getNearestEnabledItemByDirection(this._topItem + this.properties.visibleItems, 'up');
        ...
        case 'bottomclose' :
            ...
            var nei = this._getNearestEnabledItemByDirection(this.dataList.itemCount - 1, 'up');
        ...
        default :
            ...
            var currentRelativeFocussedIndex = this._getRelativeFocussedIndex();

            // get future absolute focussed index
            var futureAbsoluteFocussedIndex = this.m.min(this._topItem + this.properties.visibleItems + currentRelativeFocussedIndex, this.dataList.itemCount-1);
            ...
            if (!this.dataList.items[futureAbsoluteFocussedIndex].disabled)
            {
                ...
                var newPos = -(this._topItem + this.properties.visibleItems) * this.properties.itemHeight;      // calculate new position
                newPos = this.m.max(newPos, this._maxScrollY);                      // constrain it to the max scroll
                this._performScroll(newPos);                                        // do the scroll
            ...
            else
            {
                ...
                var nei = this._getNearestEnabledItemByDirection(this._topItem + this.properties.visibleItems, 'down');
                if (null != nei)
                {
...
List2Ctrl.prototype._scrollUpPage = function()
{
    ...
    switch (listPosition)
    {
        ...
        case 'top' :
            ...
            var nei = this._getNearestEnabledItemByDirection(-1, 'down');
        ...
        case 'topclose' :
            ...
            var nei = this._getNearestEnabledItemByDirection(0, 'down');
            if (null != nei && nei <= this.properties.visibleItems-1)
            {
        ...
        default :
            ...
            // get future absolute focussed index
            var futureAbsoluteFocussedIndex = this.m.max(this._topItem - this.properties.visibleItems + currentRelativeFocussedIndex, 0);
...
List2Ctrl.prototype._performScroll = function(pos, duration)
{
    ...
    {
        // make it snappy
        var newPos = this._getSnapPosition(pos);
...
List2Ctrl.prototype._abortScroll = function(e)
{
    ...
    var snapPos = this._getSnapPosition(this.scroller.offsetTop);
...
List2Ctrl.prototype._startSwipe = function(distance, time)
{
    ...
    var momentumY = this._momentum(distance, time, -this._y, this._maxScrollY < 0 ? this._scrollerH - this._maskH + this._y - this._minScrollY : 0, 0);

    /* ANIMATE THE SCROLLER */
    var newPos = this.m.min(this.m.max(this._y + momentumY.dist, this._maxScrollY), 0);
    var swipeDuration = momentumY.time;
...
{
    // physics calculations
    var momentumY = this._momentum(distance, time, -this._yIndex, this._maxScrollYIndex < 0 ? this._scrollerHIndex - this._maskH + this._yIndex - this._minScrollYIndex : 0, 0);

    /* ANIMATE THE LETTER INDEX SCROLLER */
    var newPos = this.m.min(this.m.max(this._yIndex + momentumY.dist, this._maxScrollYIndex), 0);
    var swipeDuration = momentumY.time;
...
{
    var deceleration = this.properties.deceleration,
        speed = this.m.abs(dist) / time,
...
List2Ctrl.prototype._letterIndexSelect = function(letterIndex, eventCause)
{
    ...
    var eventCause = ('Multicontroller' != eventCause && 'Touch' != eventCause) ? null : eventCause;
    // produce beep
...
List2Ctrl.prototype._snapIndex = function(pos)
{
    ...
    if (pos == this._maxScrollYIndex)
    {
        // set top item and bring focus on the screen
        this._setTopLetterIndex(pos);
        var focussedIndex = this._getFocussedLetterIndex();
    ...
    var snapPos = this._getIndexSnapPosition(pos);
...
List2Ctrl.prototype._scrollToIndex = function(letter)
{
    ...
    else if (typeof letter === 'string');
    {
        ...
        for (var i=0, l=this.letterIndexData.length; i<l; i++)
        {
    ...
    // do the scroll
    var newPos = -(targetIndex) * this.properties.letterIndexHeight; // calculate new position
...
List2Ctrl.prototype._scrollDownOneIndex = function()
{
    ...
    {
        var bi = this._getNearestEnabledLetterByDirection(this._topLetterIndex+this.properties.visibleLetterIndexItems-2, 'down');
        // do not scroll if no enabled letters are found
        if (null != bi)
        {
            var newTopLetter = bi + 2 - this.properties.visibleLetterIndexItems;
...
List2Ctrl.prototype._scrollUpOneIndex = function()
{
    ...
    {
        var bi = this._getNearestEnabledLetterByDirection(this._topLetterIndex+1, 'up');
        // do not scroll if no enabled items are found
        if (null != bi)
        {
            var newTopLetter = bi - 1;
            newPos = -newTopLetter * this.properties.letterIndexHeight;
...
List2Ctrl.prototype._setLetterIndexPosition = function(index)
{
    ...
    // get the new index
    var targetIndex = -1;
    for (var i=this._letterIndexDataSorted.length-1; i>=0; i--)
    {
...
List2Ctrl.prototype._setCurrentLetterIndex = function(letter)
{
    ...
    var targetIndex = this.m.max(this.m.min(letter, this.letterIndexData.length-1), 0); // constrain the index
...
List2Ctrl.prototype._showFocusLetterIndex = function(letter)
{
    ...
    var lastFocussedIndex = this._getFocussedLetterIndex();
...
List2Ctrl.prototype._getRelativeFocussedLetterIndex = function()
{
    ...
    // get real focussed index ...
    var currentFocussedIndex = this._getFocussedLetterIndex();
...
{
    var direction = (direction != 'up' && direction != 'down') ? 'down' : direction;
    var currentLetter = ('down' === direction) ? fromLetter+1 : fromLetter-1;
    var letterIndexCount = this.letterIndexData.length;
...
List2Ctrl.prototype._scrollerAnimationEnd = function()
{
    ...
    // get scroll size
    var scrollSize = this.m.abs(this._prevTopItem - this._topItem);
...
List2Ctrl.prototype._restoreFocus = function()
{
    ...
    {
        var neiDown = this._getNearestEnabledItem(this._topItem, 'down');
...
List2Ctrl.prototype._restoreLetterIndexFocus = function()
{
    ...
    else
    {
        ...
        var neiDown = this._getNearestEnabledLetterByDirection(this._topLetterIndex, 'down');
        var neiUp = this._getNearestEnabledLetterByDirection(this._topLetterIndex, 'up');
        // determine scroll direction
        var scrollDirection = (this._topLetterIndex - this._prevTopLetterIndex < 0) ? 'up' : 'down';
...
{
    // determine target item
    var itemIndex = this._getTargetItem(e);
    ...
    {
        var relativeX = e.pageX - this._maskPositionX;
        var inHittable = false;
        var rightBoundary = this.properties.sliderReferencePointRight;
        var leftBoundary = this.properties.sliderReferencePointRight - this.properties.sliderWidth;
    }
    else if(this.dataList.items[itemIndex].indented)
    {
        var relativeX = e.pageX - (Math.ceil(this._maskPositionX / 1.5));
        var inHittable = false;
        var rightBoundary = this.properties.sliderReferencePointRight - (Math.ceil(this.properties.sliderWidth / 1.5)) + (this.properties.indentOffset * 2);
        var leftBoundary = this.properties.sliderReferencePointLeft;
    } else
    {
        var relativeX = e.pageX - (Math.ceil(this._maskPositionX / 1.5));
        var inHittable = false;
        var rightBoundary = this.properties.sliderReferencePointRight - (Math.ceil(this.properties.sliderWidth / 1.5));
        var leftBoundary = this.properties.sliderReferencePointLeft;
    }
    ...
    var sliderInstance = this._getSlider(itemIndex);
    var skipActiveSlider = (true === skipActiveSlider);
    if (!skipActiveSlider)
    {
...
List2Ctrl.prototype._slideMove = function(e)
{
    ...
    // determine target item
    var itemIndex = this._activeSlider.itemIndex;
...
List2Ctrl.prototype._slideEnd = function(e)
{
    ...
    if (!this._activeSlider)
    {
        var sliderInstance = this._slideStart(e, true);
...
List2Ctrl.prototype._slideCallback = function()
{
    ...
    var itemIndex = arguments[0];

    // get value and final adjustment from fourth argument
    var value = arguments[3].value;
    var finalAdjustment = arguments[3].finalAdjustment;
...
{
    var targetElt = this._getDOMItem(itemIndex);
    if (targetElt)
    {
        var hitItems = targetElt.querySelectorAll('.hit');
...
List2Ctrl.prototype._buttonMakeHit = function(e)
{
    ...
    // Check if we are in the hittable area
    var inHittable = false;
    var rightBoundary = this.properties.toggleReferencePointRight;
    var leftBoundary = 0;
    switch (this.dataList.items[itemIndex].itemStyle)
    {
    ...
    var clickedZone = (relativeX - leftBoundary) / (rightBoundary - leftBoundary);
    var buttonId = null;
    switch (this.dataList.items[itemIndex].itemStyle)
    {
        case 'style10' :
    ...
    if (buttonId)
    {
        ...
        var domItem = this._getDOMItem(itemIndex);
        var buttons = domItem.querySelectorAll('.button');
        for (var i=0; i<buttons.length; i++)
        {
...
List2Ctrl.prototype._buttonSelectLeft = function(itemIndex)
{
    ...
    var current = this.dataList.items[itemIndex].value;
...
List2Ctrl.prototype._buttonActivate = function(itemIndex, buttonId)
{
    ...
    {
        var buttonId = (!isNaN(buttonId)) ? buttonId : 1;
        if (buttonId > 2)
            buttonId = 1;
        else if (buttonId < 1)
            buttonId = 2;
...
List2Ctrl.prototype._lockMakeHit = function(e)
{
    ...
    var inHittable = false;
    var domItem = this._getDOMItem(itemIndex);
    var lockButton = domItem.querySelector('.buttonLock');
    var deleteButton = domItem.querySelector('.buttonDelete');
    var leftBoundary = lockButton.offsetLeft;
    var rightBoundary;
...
List2Ctrl.prototype._lockMoveFocusLeft = function(itemIndex)
{
    ...
    var current = this._lockGetFocus(itemIndex);
...
List2Ctrl.prototype._lockShowFocus = function(itemIndex, buttonId)
{
    ...
    {
        // validate button id
        var buttonId = this.dataList.items[itemIndex].locked ? 1 : this.m.min(this.m.max(buttonId, 1), 2);
...
List2Ctrl.prototype._stepAdjust = function(e)
{
    ...
    // perform hit test
    var itemDOMElement = this._getDOMItem(itemIndex);
    if (!itemDOMElement)
    {
        return;
    }

    var p = itemDOMElement.querySelector('.plus');
    var m = itemDOMElement.querySelector('.minus');
    var pLayout = { x1:p.offsetLeft, x2:p.offsetLeft + p.clientWidth };
    var mLayout = { x1:m.offsetLeft, x2:m.offsetLeft + m.clientWidth };
...
List2Ctrl.prototype._stepUp = function(itemIndex)
{
    ...
    var oldValue = this.dataList.items[itemIndex].value;
    var newValue = this.m.min(this.dataList.items[itemIndex].value + this.dataList.items[itemIndex].increment, this.dataList.items[itemIndex].max);
...
List2Ctrl.prototype._stepDown = function(itemIndex)
{
    ...
    var oldValue = this.dataList.items[itemIndex].value;
    var newValue = this.m.max(this.dataList.items[itemIndex].value - this.dataList.items[itemIndex].increment, this.dataList.items[itemIndex].min);
...
List2Ctrl.prototype._releaseListReorder = function(preventSelect)
{
    ...
    var draggableItemIndex = draggableItems[0];
    ...
    var preventSelect = Boolean(preventSelect);
...
List2Ctrl.prototype._startReorder = function(e)
{
    ...
    if (itemIndex === this._reorderCurrentIndex)
    {
        ...
        var positiveButtonHit = this._buttonMakeHit(e);
        ...
        if (!positiveButtonHit)
        {
            ...
            var tmp = this._getDOMItem(itemIndex);
            ...
            var ghostItem = {itemStyle:'ghost', hasCaret:false};
...
List2Ctrl.prototype._moveReorder = function(e)
{
    if (this._reorderTouchElt)
    {
        ...
        var newPos = (e.pageY - this._maskPositionY) + this.m.abs(this.scroller.offsetTop) - (this.properties.itemHeight / 2);
        ...
        // get last move
        var moveDirection = this._getMoveDirection();
...
List2Ctrl.prototype._endReorder = function(e)
{
    if (this._reorderTouchElt)
    {
        ...
        var newSnappedIndex = Math.floor( ( (e.pageY - this._maskPositionY) + this.m.abs(this.scroller.offsetTop) ) / this.properties.itemHeight );

        // get mouse position relative to scroller corrected with the reorder touch element position
        var newPos = newSnappedIndex * this.properties.itemHeight;
...
List2Ctrl.prototype._bringReorderItem = function()
{
    ...
    {
        // get last event
        var lastEvent = this._trackedEvents[this._trackedEvents.length-1];

        // get mouse position relative to scroller corrected with the reorder touch element position
        var newPos = (lastEvent.y - this._maskPositionY) + this.m.abs(this.scroller.offsetTop) - (this.properties.itemHeight / 2);
...
List2Ctrl.prototype._reorderItemDown = function(reorderCount)
{
    ...
    for (var count = 1; count <= reorderCount; count++)
    {
        // get draggable item index
        var draggableItemIndex = this.getItemsByType('draggable')[0];

        // get new index not exceeding the list count
        var targetItemIndex = this.m.min(draggableItemIndex + 1, this.dataList.itemCount - 1);

        // reorder the dataList.items array
        var tempCopy = this.dataList.items[targetItemIndex];
...
List2Ctrl.prototype._reorderItemUp = function(reorderCount)
{
    ...
    for (var count = 1; count <= reorderCount; count++)
    {
        ...
        // get new index not exceeding the list count
        var targetItemIndex = this.m.max(draggableItemIndex - 1, 0);
...
List2Ctrl.prototype._reorderGhostItemDown = function()
{
    ...
    // get draggable item index
    var ghostItemIndex = this.getItemsByType('ghost')[0];

    // get new index not exceeding the list count
    var targetItemIndex = this.m.min(ghostItemIndex + 1, this.dataList.itemCount - 1);
...
List2Ctrl.prototype._reorderGhostItemUp = function()
{
    ...
    // get new index not exceeding the list count
    var targetItemIndex = this.m.max(ghostItemIndex - 1, 0);
...
List2Ctrl.prototype._dispatch = function(type, data)
{
    if (!type || '' == type)
        return;
    var evt = new CustomEvent( type, { detail : { data : data, bubbles: true, cancelable: true } } );
...
List2Ctrl.prototype.updateItems = function(firstItem, lastItem)
{
    ...
    else if (firstItem >= 0 && lastItem >= 0 && lastItem >= firstItem && !this._hasFill)
    {
        ...
        var lastFillItem = this.m.min(lastItem, this.properties.itemsBefore + this.properties.itemsAfter);
        ...
        {
            var focussedItem = this.focussedItem;
            var topInFocusRange = focussedItem >= this.topItem && focussedItem < this.topItem + this.properties.visibleItems - 1;
            var prevTopInFocusRange = focussedItem >= this._prevTopItem && focussedItem < this._prevTopItem + this.properties.visibleItems - 1;
    ...
    else if (firstItem >= 0 && lastItem >= 0 && lastItem >= firstItem)
    {
        ...
        else if (this._inSecondaryMulticontroller)
        {
            ...
            var smi = this._currentSecondaryMulticontrollerItem;
...
List2Ctrl.prototype.setLetterIndexData = function(data)
{
    ...
    var additionalSpace = Math.ceil(this.properties.letterIndexHeight / 2) - 5; // adjusting factor
    ...
    if (this.hasDataList() && this._letterIndexDataSorted.length)
    {
        ...
        var focussedIndex = this._getFocussedIndex();
        var firstIndex = this._letterIndexDataSorted[0].itemIndex;
...
List2Ctrl.prototype.setLineNumbers = function()
{
    ...
    var itemCount = this.m.min(this.dataList.itemCount, maxItemCount);
...
{
    // get target item
    var targetIndex = this._topItem + (lineNumber - 1);
    ...
    else
    {
        ...
        var selectResult = this._itemSelect(targetIndex, {fromVui:true, vuiStatus:'selected'});
...
{
    var status = this._scrollDownPage();
    return status;
};
...
List2Ctrl.prototype.setSliderValue = function(itemIndex, value)
{
    ...
    var slider = this._getSlider(itemIndex);
    if (slider)
    {
...
List2Ctrl.prototype.setToggleValue = function(itemIndex, value)
{
    ...
    {
        var value = this.m.max(this.m.min(value, 2), 1);
    }
    else if(item.itemStyle == 'style11')
    {
        var value = this.m.max(this.m.min(value, 3), 1);
    }
...
List2Ctrl.prototype._setRadio = function(itemIndex, value)
{
    ...
    for (var i=0; i<this.dataList.itemCount; i++)
    {
...
{
    var item = null;
    var data = (typeof data === 'object' && data != null) ? JSON.stringify(data) : data;

    for (var i=0, l=this.dataList.items.length; i<l; i++)
    {
        var currentAppData = (typeof this.dataList.items[i].appData === 'object' && this.dataList.items[i].appData != null) ? JSON.stringify(this.dataList.items[i].appData) : this.dataList.items[i].appData;
...
List2Ctrl.prototype.setTitle = function(titleStructure)
{
    ...
    // prepare title
    var titleStructure = titleStructure || {};
...
List2Ctrl.prototype.setListBackground = function(img, position)
{
    ...
    {
        var left = (!isNaN(position['left'])) ? position.left + 'px' : position.left.toString();
        var top = (!isNaN(position['top'])) ? position.top + 'px' : position.top.toString();
...
{
    // get item index
    var itemIndex = (undefined === itemIndex) ? this._topItem : itemIndex;
...
List2Ctrl.prototype._getTargetItem = function(e)
{
    ...
    var relativeY = (e.pageY - this._maskPositionY) - this.scroller.offsetTop;
    ...
    var itemIndex = Math.floor(relativeY / this.properties.itemHeight);
...
{
    // determine touched item
    var relativeY = (e.pageY - this._maskPositionY) - this.letterIndex.offsetTop;
    ...
    var letterIndex = Math.floor(relativeY / this.properties.letterIndexHeight);
...
List2Ctrl.prototype._getSlider = function(itemIndex)
{
    ...
    var domElt = this._getDOMItem(index);
    if (domElt)
    {
        var poolId = domElt.getAttribute('data-poolid');
        var hashKey = 'slider_'+index+'_'+poolId;
...
List2Ctrl.prototype._wrapInlineElement = function(li)
{
    ...
    var line1 = li.getElementsByClassName(searchClass);
...
List2Ctrl.prototype._displayWithoutText = function(item)
{
    ...
    for(var i =0; i < this._itemsWithNoText.length; i++)
    {
...
List2Ctrl.prototype._hasRightHittableArea = function(item)
{
    ...
    for(var i =0; i < this._rightHittableArea.length; i++)
    {
...
List2Ctrl.prototype.cleanUp = function()
{
    ...
    var params = { scrollTo : this._topItem, focussedItem : this._lastItemWithFocus };
