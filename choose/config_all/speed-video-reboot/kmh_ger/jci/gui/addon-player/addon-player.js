...
$(document).ready(function(){
	...
	function myVideoPlaybackManage(obj){
		if(playbackStatus){
			$.get('/tmp/root/playbackStatus', function (data) {
				if(data.length > 2){
	...
	function myVideoLoadList(){
		$.get('/tmp/root/myVideoList', function (data) {
			if(data.length < 2){
	...
	function getMyVideoPlaybackOption(){
		// this will be per startup now - maybe find a persistant location that is R/W by default
		$.get('/tmp/root/playbackOption', function (data) {
